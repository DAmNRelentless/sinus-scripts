/*
    ...
    Requested by: Mortis
    License: LGPL-3.0
    Repository: https://github.com/DAmNRelentless/sinus-scripts
...
registerPlugin(
    {
        ...
        vars: [
            ...
            {
                name: 'functionality',
                title: "The script stores usernames of clients with relevant groups. Each client you want to list has to join the server at least once while the script is running. A client can't be displayed in the staff list if they are offline and are not stored yet."
            },
            {
                name: 'configuration',
                title: 'A guide how to configure the script to your needs can be found here: https://github.com/DAmNRelentless/sinus-scripts/wiki'
            },
            {
                name: 'spacer0',
                title: ''
            ...
            },
            {
                ...
                type: 'string',
                placeholder:
                    'Not enough arguments! You have to add a client UID to the command! Usage: !remove <client UID>',
                indent: 1,
                conditions: [
                    {
                        field: 'removeCommand',
            ...
            },
            {
                ...
                title: "Not-Found-Text > Define the text that should be sent to the command invoker if the target client couldn't be found in the database! | placeholders: %uid% - uid of the target client",
                type: 'string',
            ...
            },
            {
                ...
                title: 'Success-Text > Define the text that should be sent to the command invoker if the database was successfully dropped/removed! | placeholders: %amount% - amount of database removals',
                type: 'string',
            ...
            {
                name: 'tGroupSection',
                title: 'Group-Section > Define what a group section in the staff list should look like! | placeholders: %group% - formatted group name, %users% - formatted member list, %lb% - line break',
                type: 'multiline',
    ...
    (_, scriptConfig) => {
        ...
        const remove = {
            ...
            rPerm: config.remove
                ? scriptConfig.commandNoPerm || "You don't have permission to perform this command!"
                : undefined,
            rArgument: config.remove
                ? scriptConfig.commandArgument ||
                  'Not enough arguments! You have to add a client UID to the command! Usage: !remove <client UID>'
                : undefined,
            rInvalid: config.remove
                ? scriptConfig.commandInvalid || '"%arg%" is not a valid client UID! Make sure to send the correct one.'
                : undefined,
            rNotFound: config.remove
                ? scriptConfig.commandNotFound ||
                  'The client (%uid%) was not found in the database! Make sure to send the correct UID.'
                : undefined,
            rSuccess: config.remove
                ? scriptConfig.commandSuccess || 'The client (%uid%) was successfully removed!'
                : undefined
        };
        const dbRemove = {
            ...
            dbrPerm: config.dbRemove
                ? scriptConfig.dbCommandNoPerm || "You don't have permission to perform this command!"
                : undefined,
            dbrEmpty: config.dbRemove ? scriptConfig.dbCommandEmpty || 'The database is already empty!' : undefined,
            dbrSuccess: config.dbRemove
                ? scriptConfig.dbCommandSuccess ||
                  'The database was successfully dropped! %amount% entries have been removed.'
                : undefined
        };
        const template = {
            ...
            section: config.template
                ? scriptConfig.tGroupSection || '[center]%group%\n%users%____________________[/center]'
                : undefined,
            pOnline:
                (config.template ? scriptConfig.tPhraseOnline : scriptConfig.phraseOnline) ||
                '[COLOR=#00ff00][B]ONLINE[/B][/COLOR]',
            pAway:
                (config.template ? scriptConfig.tPhraseAway : scriptConfig.phraseAway) ||
                '[COLOR=#c8c8c8][B]AWAY[/B][/COLOR]',
            pOffline:
                (config.template ? scriptConfig.tPhraseOffline : scriptConfig.phraseOffline) ||
        ...
        const emptyGroup = {
            emptyText: config.emptyGroup
                ? scriptConfig.emptyGroupText ||
                  '[COLOR=#aa007f][size=12][B]%group%[/B][/size]\n[/COLOR][COLOR=#c8c8c8][B]NOT ASSIGNED[/B][/COLOR]'
                : undefined
        };
        const staticText = {
            headerText: config.header
                ? scriptConfig.headerText ||
                  'This is the staff list of the server!\nYou can see if a specific team member is online, afk or offline.'
                : undefined,
        ...
        /**
         * Wait for the backend to be online/connected each given amount of time
         * for a given amount of tries.
         *
         * @param   {number}  attempts The amount of tries the function should
         *   check for the backend to be online/connected.
         * @param   {number}  wait     The amount of time (in seconds) that
         *   should be waited between each try.
         * @returns {Promise}          Resolve when the backend is
         *   online/connected, reject when the backend was not online/connected in time.
         */
            ...
            return new Promise((success, fail) => {
                ...
                const timer = setInterval(() => {
                    if (backend.isConnected()) {
                        ...
                        if (config.dev)
                            log(
                                'waitForBackend() failed at ' +
                                    attempt +
                                    '. attempt with a timer of ' +
                                    wait +
                                    ' seconds'
                            );
                        fail('backend');
        ...
        /**
         * Check all staff groups from the config if their entries are valid and
         * if all IDs reference valid objects on TeamSpeak; Otherwise drop the
         * whole config entry for the runtime.
         *
            ...
            config.staffGroups.forEach((group, index) => {
                ...
                if (group.groups === undefined || !group.groups.length) {
                    ...
                    group.groups = group.groups.filter(
                        id => backend.getServerGroupByID(id) !== undefined && id !== group.id
                    );
        ...
        /**
         * Validate the script database by checking all entries if they still
         * have a relevant group to list and if they match the current database
         * format; Otherwise drop the whole entry.
         *
            ...
            store.getKeys().forEach(key => {
                ...
                if (Array.isArray(store.get(key)[1])) {
                    if (store.get(key)[1].some(clientGroup => typeof clientGroup !== 'object')) {
                        ...
                        // get relevant groups
                        const relevantGroups = store
                            .get(key)[1]
                            .filter(clientGroup => groupList.includes(clientGroup.id));
        ...
        /**
         * Store a client to the script's database if they are not already
         * stored; Can also update information if entry is alreadyp present;
         *
        ...
        /**
         * Remove a client from the script's database if they are stored; Will
         * give feedback if the entry was removed.
         *
         * @param   {string}  uid The UID of the client to remove.
         * @returns {boolean}     True if the client was removed, false if no
         *   entry was found.
         */
        ...
        /**
         * Update the global list of staff clients with an easier to access
         * format; Accessing a variable is also faster than reading everything
         * from the script storage.
         *
         ...
         * @returns {boolean}            True if the client is a valid member,
         *   otherwise false.
         */
        function isStaffGroupMember(client, staffGroup) {
            if (staffGroup.clients.includes(client.uid())) return true;

            for (const clientGroup of client.getServerGroups()) {
         ...
         *
         * @param   {object}  client The client object to check.
         * @returns {boolean}        True if the client is counted as away/afk,
         *   otherwise false.
         */
        function isAway(client) {
            return (
                client.isAway() ||
                (config.awayMute && client.isMuted()) ||
                (config.awayDeaf && client.isDeaf()) ||
                (config.awayChannel && isInAfkChannel(client))
            );
        }
         ...
         *
         * @param   {object}  client The client object to check.
         * @returns {boolean}        True if the client is in any afk channel,
         *   otherwise false.
         */
         ...
         *
         * @param   {Array}  staffClient The staff client array [uid, nickname,
         *   [staff groups]]
        ...
        /**
         * Format a whole line that will be displayed in the list by adding all
         * small parts together.
         *
        ...
        function getFormattedUserLine(name, status) {
            let formattedLine = '';
            if (config.template) {
                formattedLine = config.line
                    .replace('%name%', config.username.replace('%name%', name))
                    .replace('%lb%', '\n');
            } else {
        ...
        event.on('load', () => {
            ...
            if (!engine.version().includes('1.0.0')) {
                ...
                return log(
                    'There are no staff groups configured to be displayed in the staff list! Deactivating script...'
                );
            } else {
                // error prevention that needs feature deactivation
                if (config.away && config.awayChannel && !config.afkChannels) {
                    log(
                        'There were no afk channels set up although the afk channel option is enabled! Deactivating the feature...'
                    );
                    config.awayChannel = false;
                }
                if (config.remove && !config.rClients.length && !config.rGroups.length) {
                    log(
                        "There are no clients whitelisted for the remove command although it's enabled! Deactivating feature..."
                    );
                    config.remove = false;
                } else if (config.remove && !config.rServer && !config.rChannel && !config.rPrivate) {
                    log(
                        'There is no text channel selected for the bot to listen to the remove command! Deactivating feature...'
                    );
                    config.remove = false;
                }
                if (config.dbRemove && !config.dbrClients.length && !config.dbrGroups.length) {
                    log(
                        "There are no clients whitelisted for the database remove command although it's enabled! Deactivating feature..."
                    );
                    config.dbRemove = false;
                } else if (config.dbRemove && !config.dbrServer && !config.dbrChannel && !config.dbrPrivate) {
                    log(
                        'There is no text channel selected for the bot to listen to the database remove command! Deactivating feature...'
                    );
                    config.dbRemove = false;
                }
                ...
                waitForBackend(10, 3)
                    ...
                    .catch(error => {
                        if (error === 'backend') {
                            ...
                            log('Unknown error occured! Please report this to the script author.');
                            console.log(error);
                        }
                    });
            }
        ...
        function main() {
            ...
            if (!staffGroups.length)
                return log(
                    'There are no valid staff groups set in your script configuration! There might be further output in the log. Deactivating script...'
                );
            ...
            /**
             * MOVE EVENT fired when a client switches channels or joins/leaves
             * the server check all necessary information here such as if the
             * client is relevant for the staff list and update the list accordingly.
             */
            ...
            /**
             * AFK EVENT fired when a client sets themself to away in TeamSpeak
            ...
            /**
             * UN-AFK EVENT fired when a client sets themself to be back in
             * TeamSpeak if the away status is activated, this will update the list.
             */
            ...
            /**
             * MUTE EVENT fired when a client mutes their microphone in
             * TeamSpeak this does not include a disabled microphone if the away
             * status is activated, this will update the list.
             */
            ...
            /**
             * UN-MUTE EVENT fired when a client unmutes their microphone in
             * TeamSpeak if the away status is activated, this will update the list.
             */
            ...
            /**
             * DEAF EVENT fired when a client sets themself to deaf in TeamSpeak
             * this does not include a disabled speakers if the away status is
             * activated, this will update the list.
             */
            ...
            /**
             * UN-DEAF EVENT fired when a client sets themself to undeaf in
             * TeamSpeak if the away status is activated, this will update the list.
             */
            ...
            /**
             * GROUP-ADDED EVENT fired when a client is added to a servergroup
             * in TeamSpeak this won't fire if the client is not visible for the
             * bot check if the group is relevant, update the client entry in
            ...
            /**
             * GROUP-REMOVED EVENT fired when a client is removed from a
             * servergroup in TeamSpeak this won't fire if the client is not
             * visible for the bot check if the group is relevant, update the
             * client entry in the database and update the list.
             */
            ...
            /**
             * CHAT EVENT fired when a client sends a visible message to the bot
             * mode: 1 - private, 2 - channel, 3 - server check for command input.
             */
            event.on('chat', event => {
                const client = event.client;
                if (client.isSelf()) return;
