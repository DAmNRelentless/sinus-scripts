...
registerPlugin(
    {
        ...
        vars: [
            ...
            {
                name: 'placeholders',
                title: 'Available placeholders: %name% - nickname, %uid% - unique id, %ip% - ip address, %country% - location, %fcon% - first connect, %tcon% - total connections, %ver% - client version, %os% - operating system/platform'
            },
            {
                name: 'message',
    ...
    (_, config) => {
        ...
        function getFcon(client) {
            const unix = client.getCreationTime();
