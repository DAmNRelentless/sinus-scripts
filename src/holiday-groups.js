...
registerPlugin(
        ...
        vars: [
            ...
            },
            {
                ...
                vars: [
                    ...
                    {
                        name: 'message',
                        title: 'Message > Define the message the client should get when the group(s) get(s) assigned! Keep in mind that poke messages can only be 100 characters long. [*] | placeholders: %name% - client name, %amount% - amount of assigned groups, %lb% - line break',
                        indent: 2,
                        type: 'multiline',
    ...
    (_, scriptConfig) => {
        ...
        /**
         * Check all holiday groups from the config if their entries are valid
         * and if all IDs reference valid objects on TeamSpeak; Otherwise drop
         * the whole config entry for the runtime.
         *
        ...
        /**
         * Handles the initial client processing when the bot connects or the
         * date changes.
         */
        ...
        /**
         * Handles the whole process of checking a client on specific events The
         * function does blacklist checks, adds the client to the specific
         * groups and also notifies the client when groups were assigned to them.
         *
         * @param {any} client The client to process.
         */
            ...
            fittingEntries.forEach(entry => {
                if (entry.accessControl) {
                    if (entry.accessControlType == 0) {
                        if (
                            !entry.accessControlClients.includes(client.uid()) &&
                            !entry.accessControlGroups.some(group => clientGroups.includes(group))
                        )
                            return;
                    } else {
                        if (
        ...
        function main() {
            ...
            if (!groups.length)
                return log(
                    'There are no valid holiday groups set in your script configuration! There might be further output in the log. Deactivating script...'
                );
