...
registerPlugin(
            ...
            },
            {
                ...
                vars: [
                    ...
                    },
                    {
                        ...
                        indent: 1,
                        options: [
                            'all trigger groups have to be added/removed',
                            'at least one trigger group has to be added/removed'
                        ]
                    },
                    {
                        name: 'triggerBot',
    ...
    (_, scriptConfig) => {
        ...
        /**
         * Check all groups from the config if their entries are valid;
         * otherwise drop the whole config.
         *
        ...
        function validateGroups(groups) {
            ...
            groups.forEach((group, index) => {
                ...
                if (group.blacklistGroups.length)
                    group.blacklistGroups = group.blacklistGroups.filter(
                        gid => backend.getServerGroupByID(gid) !== undefined
                    );

                // deactivate advanced conditions if non are set
                if (group.advancedConditions && !group.blacklistClients.length && !group.blacklistGroups.length)
                    group.advancedConditions = false;
         ...
         *
         * @param {object} client The object of the client the groups should be
         *   assigned to.
        ...
        /**
         * Handle the required events for the script Since all events do
         * basically the same, this is done in a function The function checks
         * all set up groups from the config and if the specific condition matches.
         *
         * @param {object} event        The event object to receive information from.
         * @param {number} trigger      The trigger type, 0 = adding, 1 = removing.
         * @param {Array}  groupsAdd    The list of all servergroups that are
         *   added to a client.
         * @param {Array}  groupsRemove The list of all servergroups that are
        ...
        function handleEvent(event, trigger, groupsAdd, groupsRemove) {
            ...
            groupsAdd.forEach(group => {
                ...
                if (
                    group.advancedConditions &&
                    group.blacklistGroups.some(blacklistGroup => clientGroups.includes(blacklistGroup))
                )
                    return;
                // if the condition requires all groups, check if the client matches it
                if (
                    group.triggerCondition == 0 &&
                    group.triggerGroups.some(
                        triggerGroup => triggerGroup !== serverGroupID && !clientGroups.includes(triggerGroup)
                    )
                )
                    return;
        ...
        event.on('load', () => {
            ...
            if (
                (!config.groupsAdd || !config.groupsAdd.length) &&
                (!config.groupsRemove || !config.groupsRemove.length)
            ) {
        ...
        function main() {
            ...
            if (!groupsAdd.length && !groupsRemove.length)
                return log(
                    'There are no valid groups set in your script configuration! There might be further output in the log. Deactivating script...'
                );
            ...
            /**
             * SERVER GROUP ADDED EVENT fired when a servergroup is assigned to
             * a client since two events are required for the whole logic and
             * both do basically the same we handle the whole process in a function.
             */
            ...
            /**
             * SERVER GROUP REMOVED EVENT fired when a servergroup is removed
             * from a client since two events are required for the whole logic
             * and both do basically the same we handle the whole process in a function.
             */
