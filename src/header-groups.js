...
registerPlugin(
    {
        ...
        version: '2.0.1',
        description:
            'With this script, the bot will automatically give the respective header groups if at least one of the trigger groups is assigned.',
        author: 'Relentless',
        backends: ['ts3'],
        vars: [
            {
    ...
    (_, config) => {
        ...
        function main() {
            ...
            event.on('serverGroupRemoved', event => {
                ...
                for (const entry of groups) {
                    if (entry.trigger.includes(group.id())) {
                        if (!entry.trigger.some(gid => getClientGroupIDs(client).includes(gid)))
                            removeServerGroup(client, entry.header);
                        if (!multipleHeaders) return;
                    }
                }
